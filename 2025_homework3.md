...
We will also use **TF-IDF** and **Singular Value Decomposition** (**SVD**) to create **embeddings from texts.** You can refer to our ["Create Your Own Search Engine" workshop](https://github.com/alexeygrigorev/build-your-own-search-engine) if you want to know more about it.
...
- 0.25
- **0.35** <-- **ANSWER!**. So the VECTOR SEARCH actually isn't better!
- 0.45
- 0.55
...
- 0.62
- 0.72
- **0.82** <-- very similar to `minsearch`
- 0.92
...
- In the **second** part of the module, we looked at evaluating the entire RAG approach. In particular, we looked at comparing the answer generated by our system with the actual answer from the FAQ. One of the ways of doing it is using the **cosine similarity.** Let's see how to calculate it.
...
For **normalization**, we first compute the vector norm (its length), and then divide the vector by it (where `np` is `import numpy as np`): 
...
When creating **embeddings,** we will use a simple way -the same we used in the [Embeddings](#embeddings) section:
...
This is a set of metrics that compares two answers based on the overlap of n-grams, word sequences, and word pairs. It can give a more nuanced view of text similarity than just cosine similarity alone.
